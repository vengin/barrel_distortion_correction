  ...
  for (int y = 0; y < IMG_HEIGHT && !file.eof(); y++) {
    for (int x = 0; x < IMG_WIDTH && !file.eof(); x++) {
      if (file >> hex_color) {
        ...
        // Debug print for read pixels (first few and problematic corners)
        if ((y == 0 && x < 5) || (y == 0 && x == IMG_WIDTH - 1) ||
            (y == IMG_HEIGHT - 1 && x == 0) || (y == IMG_HEIGHT - 1 && x == IMG_WIDTH - 1) ||
            (y == IMG_HEIGHT / 2 && x == IMG_WIDTH / 2)) {
          std::cout << "DEBUG: Read pixel [" << y << "][" << x << "]: hex=" << hex_color << ", gray=" << gray << std::endl;
        }
      } else {
        std::cout << "Warning: Not enough pixel data in file. Filling remaining with black." << std::endl;
...
int main() {
  ...
  // Debug prints for input image
  std::cout << "\n--- Input Image Pixel Check ---" << std::endl;
  std::cout << "Input[0][0]: " << (int)input_img[0][0] << std::endl;
  std::cout << "Input[0][" << IMG_WIDTH - 1 << "]: " << (int)input_img[0][IMG_WIDTH - 1] << std::endl;
  std::cout << "Input[" << IMG_HEIGHT - 1 << "][0]: " << (int)input_img[IMG_HEIGHT - 1][0] << std::endl;
  std::cout << "Input[" << IMG_HEIGHT - 1 << "][" << IMG_WIDTH - 1 << "]: " << (int)input_img[IMG_HEIGHT - 1][IMG_WIDTH - 1] << std::endl;
  std::cout << "Input[" << IMG_HEIGHT / 2 << "][" << IMG_WIDTH / 2 << "]: " << (int)input_img[IMG_HEIGHT / 2][IMG_WIDTH / 2] << std::endl;
  ...
  // Debug prints for output image
  std::cout << "\n--- Output Image Pixel Check ---" << std::endl;
  std::cout << "Output[0][0]: " << (int)output_img[0][0] << std::endl;
  std::cout << "Output[0][" << IMG_WIDTH - 1 << "]: " << (int)output_img[0][IMG_WIDTH - 1] << std::endl;
  std::cout << "Output[" << IMG_HEIGHT - 1 << "][0]: " << (int)output_img[IMG_HEIGHT - 1][0] << std::endl;
  std::cout << "Output[" << IMG_HEIGHT - 1 << "][" << IMG_WIDTH - 1 << "]: " << (int)output_img[IMG_HEIGHT - 1][IMG_WIDTH - 1] << std::endl;
  std::cout << "Output[" << IMG_HEIGHT / 2 << "][" << IMG_WIDTH / 2 << "]: " << (int)output_img[IMG_HEIGHT / 2][IMG_WIDTH / 2] << std::endl;
