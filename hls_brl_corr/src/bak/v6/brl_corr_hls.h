...
public:
  ...
  pixel_t read_pixel(int line_offset, int x) {
    #pragma HLS INLINE
    if (x < 0 || x >= IMG_WIDTH) { // Only check x bounds here
      return 0;
    }
    // Ensure positive modulo for read_idx, allowing negative line_offset for older lines
    int read_idx = (write_idx + line_offset);
    // Handle negative results from C++ % operator for negative dividends
    read_idx = (read_idx % LINE_BUFFER_SIZE + LINE_BUFFER_SIZE) % LINE_BUFFER_SIZE;
